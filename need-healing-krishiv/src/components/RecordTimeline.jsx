...
function RecordCard({ rec, index, allowDeleteRequest, onRequestDelete, deletingRecordId }) {
    ...
    >
      <div className="timeline-record-header">
        ...
        <div className="timeline-record-right">
          ...
          <div className="timeline-record-chevron">{expanded ? 'Hide' : 'Show'}</div>
        </div>
      </div>
            ...
            <>
              ...
              {allowDeleteRequest && (
                <div className="mt-4 flex justify-end">
                  ...
                  >
                    {deletingRecordId === rec.id ? 'Sending...' : '🗑️ Request Delete'}
                  </button>
                </div>
              )}
            </>
...
// ─── MAIN COMPONENT ───────────────────────────────────────────────────────────
export default function RecordTimeline({ userId, showAddButton, onAddRecord, allowDeleteRequest = false, onRequestDelete = null, deletingRecordId = null }) {
  ...
  return (
    <div className="record-timeline">
      ...
      <div className="timeline-filters">
        {typeKeys.map(key => {
          const meta = key === 'all' ? { icon: 'All', label: 'All', color: 'var(--primary)' } : TYPE_META[key];
              ...
              <div className="timeline-date-records">
                {recs.map((rec, i) => (
                  <RecordCard
                    ...
                    rec={rec}
                    index={i}
                    allowDeleteRequest={allowDeleteRequest}
