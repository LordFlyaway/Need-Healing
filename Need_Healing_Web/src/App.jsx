...
export default function App() {
  ...
  const [currentUser, setCurrentUser] = useState({ uid: 'mock123', role: 'patient', name: 'John Doe', patient_id: 'JOHN123', public_id: 'JD_PUBLIC', blood_group: 'O+', emergency_contact: { name: 'Jane Doe', phone: '123-456-7890' } });
      ...
      <aside className="w-[240px] flex flex-col fixed inset-y-0 left-0 z-50 bg-nav border-r border-white/5 transition-colors">
        ...
        <div className="flex-1 p-3">
          <div className="p-2.5 rounded-lg bg-white/10 text-white text-[13px] font-medium flex items-center gap-2.5">
            <span>{currentUser.role === 'patient' ? '📊' : '🖥️'}</span>
            Dashboard
          </div>
          {currentUser.role === 'patient' && (
            <div 
              className="p-2.5 mt-1 rounded-lg text-white/70 hover:bg-white/10 hover:text-white cursor-pointer text-[13px] font-medium flex items-center gap-2.5 transition-all"
              onClick={() => {
                const el = document.getElementById('emergency-profile-section');
        ...
        <div className="mt-auto p-4 border-t border-white/10">
          <button onClick={() => setMode(mode === "light" ? "dark" : "light")} className="w-full flex items-center gap-2 p-2 rounded-lg text-xs font-medium bg-white/5 text-nav-text hover:bg-white/10 transition-all">
            <span>{mode === "light" ? "🌙" : "☀️"}</span> {mode === "light" ? "Dark Mode" : "Light Mode"}
          </button>
