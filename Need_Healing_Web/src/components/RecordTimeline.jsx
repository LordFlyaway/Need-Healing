...
function RecordCard({ rec, index, viewer, onRequestDelete, onEditSymptom, onDeleteSymptom }) {
        ...
        <div className="timeline-record-details">
          {decrypting ? (
            ...
            <>
              ...
              {viewer === 'doctor' && rec.type === 'prescription' && (
                <div className="mt-4 pt-4 border-t border-slate-700/50">
                  <button 
                    onClick={(e) => {
                      e.stopPropagation();
                      if (onRequestDelete) onRequestDelete(rec);
                    }}
                    className="w-full py-3 bg-red-500/20 hover:bg-red-500/40 text-red-400 font-bold rounded-xl transition-all border border-red-500/50 flex items-center justify-center gap-2"
                  >
                    <span>🗑️</span> <span>Request Deletion</span>
                  </button>
                </div>
              )}
              {viewer === 'patient' && rec.type === 'symptom' && (
                <div className="mt-4 pt-4 border-t border-slate-700/50 flex gap-4">
                  <button 
                    onClick={(e) => {
                      e.stopPropagation();
                      if (onEditSymptom) onEditSymptom(rec);
                    }}
                    className="flex-1 py-3 bg-blue-500/20 hover:bg-blue-500/40 text-blue-400 font-bold rounded-xl transition-all border border-blue-500/50 flex items-center justify-center gap-2"
                  >
                  ...
                  <button 
                    onClick={(e) => {
                      e.stopPropagation();
                      if (onDeleteSymptom) onDeleteSymptom(rec);
                    }}
                    className="flex-1 py-3 bg-red-500/20 hover:bg-red-500/40 text-red-400 font-bold rounded-xl transition-all border border-red-500/50 flex items-center justify-center gap-2"
                  >
...
// ─── MAIN COMPONENT ───────────────────────────────────────────────────────────
export default function RecordTimeline({ userId, showAddButton, onAddRecord, viewer, onRequestDelete, onEditSymptom, onDeleteSymptom }) {
