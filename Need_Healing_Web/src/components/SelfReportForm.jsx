...
const SELF_REPORT_TYPES = [
  { key: 'symptom', label: 'Current Symptom', icon: '🤒', color: '#fb923c' },
];
...
export default function SelfReportForm({ userId, onClose, initialData = null }) {
  const [selectedType, setSelectedType] = useState(initialData?.type || 'symptom');
  const [fields, setFields] = useState(initialData ? { ...initialData } : { ...INITIAL_FIELDS.symptom });
  ...
  const handleSubmit = async () => {
    const hasData = Object.values(fields).some(v => v && v.toString().trim() !== '');
    if (!hasData) return;
    ...
    try {
      ...
      if (initialData && initialData.id) {
        await updateDoc(doc(db, "users", userId, "records", initialData.id), {
          ...fields,
        });
        setSuccess('Record updated in your vault!');
      } else {
    ...
    <div className="self-report-overlay" onClick={onClose}>
      <div className="self-report-modal" onClick={e => e.stopPropagation()}>
        <div className="self-report-header">
          <h2>{initialData ? 'Edit Symptom' : 'Report Current Symptoms'}</h2>
        ...
        >
          {submitting ? 'Saving…' : (initialData ? 'Update Record' : 'Save to My Vault')}
        </button>
      </div>
    </div>
  );
